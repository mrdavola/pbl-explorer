        ...
        return (
          ...
          >
            ...
            {cluster.approaches.map((approach, ai) => {
              const aAngle = mid + (ai - (cluster.approaches.length - 1) / 2) * (approachAngleSpan / Math.max(cluster.approaches.length - 1, 1));
            ...
            <motion.line x1={cx} y1={cy} x2={clusterCx} y2={clusterCy} stroke={cluster.color} strokeWidth={1.5} strokeDasharray="4 3" initial={{ pathLength: 0, opacity: 0 }} animate={{ pathLength: 1, opacity: 0.5 }} transition={{ delay: 0.4 + qi * 0.1, duration: 0.4 }} />
          </motion.g>
        );
      })}
...
function ClusterTaxonomyMobile() {
  return (
    <div className="w-full space-y-3">
      {/* Center hub */}
      <motion.div
        className="text-center py-4 px-6 rounded-2xl font-bold text-lg"
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 px-4 pb-4">
            {cluster.approaches.map((approach, ai) => (
              <motion.span
                key={approach}
                className="inline-block px-3.5 py-2 rounded-xl text-sm font-medium"
                style={{
                ...
                animate={{ opacity: 1, scale: 1 }}
                transition={{ delay: 0.4 + i * 0.1 + ai * 0.06, duration: 0.3 }}
              >
                {approach}
              </motion.span>
            ))}
